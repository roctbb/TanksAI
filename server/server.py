    ...
    def post(self):
        try:
            ...
            if settings["mode"] == "sandbox":
                if player[3] == "waiting":
                    print(strftime("%Y-%m-%d %H:%M:%S", gmtime()) + ": " + player[2] + " (" + player[
                        1] + ") has loaded new bot")
                else:
                    print(strftime("%Y-%m-%d %H:%M:%S", gmtime()) + ": " + player[2] + " (" + player[
                        1] + ") has reloaded bot")
        ...
        for record in result:
            ...
            if action[0] == "fire_up":
                for i in range(y - 1, -1, -1):
                    if mainMap[x][i] != '.' and mainMap[x][i] != '&uarr;' and mainMap[x][i] != '&darr;' and mainMap[x][
                        i] != '&larr;' and mainMap[x][i] != '&rarr;' and mainMap[x][i] != '#' and mainMap[x][i] != '@':
                        mainMap[x][i]['hit'] = 1
                        break
            ...
            if action[0] == "fire_left":
                for i in range(x - 1, -1, -1):
                    if mainMap[i][y] != '.' and mainMap[i][y] != '&uarr;' and mainMap[i][y] != '&darr;' and mainMap[i][
                        y] != '&larr;' and mainMap[i][y] != '&rarr;' and mainMap[i][y] != '#' and mainMap[i][y] != '@':
                        mainMap[i][y]['hit'] = 1
                        break
